...
import { RampLayerConfig, LayerTypes } from 'ramp-geoapi';
...
type LayerFactory = (config: RampLayerConfig, geoJson: any, systemOptions: any) => GeoJsonLayer;

type WFSResponseJson = {
...
class ClientSideData extends LayerBlueprintMixin {
    ...
    async validate(type: string = this.type, isEncoded: boolean = true): Promise<any> {
        ...
        [error, this._validationResult] = await to(api.geoapi.layers.validateFile(type, this._rawData, isEncoded));
    ...
    async _makeLayer(force: boolean = false): Promise<any> {
        ...
        const layer = this.layerFactory(this.config, clonedFormattedData, this.config);
...
class FeatureServiceSource extends mixins(BlueprintBase, ServerSideData, FieldsOption) {
    ...
    get layerRecordFactory(): LayerRecordFactory {
        return () => new Promise(() => 'not implemented yet'); //api.geoapi.layers.createFeatureRecord;
    }
...
class DynamicServiceSource extends mixins(BlueprintBase, ServerSideData, LayersOption) {
    ...
    get layerRecordFactory(): LayerRecordFactory {
        return () => new Promise(() => 'not implemented yet'); //api.geoapi.layers.createDynamicRecord;
    }

    get type() {
        return 'DynamicService'; //LayerTypes.DynamicService;
    }
}
...
class WMSServiceSource extends mixins(BlueprintBase, ServerSideData, LayersOption) {
    ...
    get layerRecordFactory(): LayerRecordFactory {
        return () => new Promise(() => 'not implemented yet'); //api.geoapi.layers.createWmsRecord;
    }
...
class ImageServiceSource extends mixins(BlueprintBase, ServerSideData) {
    ...
    get layerRecordFactory(): LayerRecordFactory {
        return () => new Promise(() => 'not implemented yet'); //api.geoapi.layers.createImageRecord;
    }

    get type() {
        return LayerTypes.MapImageLayer; // Is this correct? or should it just be 'ImageService'
    }
}
...
class TileServiceSource extends mixins(BlueprintBase, ServerSideData) {
    ...
    get layerRecordFactory(): LayerRecordFactory {
        return () => new Promise(() => 'not implemented yet'); //api.geoapi.layers.createTileRecord;
    }
...
class WFSServiceSource extends mixins(BlueprintBase, ClientSideData, FieldsOption) {
    ...
    validate(): any {
        ...
        return super.validate(LayerTypes.GeoJSON, false).then(validationResult => {
    ...
    get layerFactory(): LayerFactory {
        return api.geoapi.layers.createGeoJSONLayer;
    }

    get layerRecordFactory(): LayerRecordFactory {
        return () => new Promise(() => 'not implemented yet'); //gapi.layer.createFeatureRecord;
    }

    get type() {
        return 'WfsLayer'; //Geo.Service.Types.WFS;
    }
    ...
    async _getWFSData(
        ...
        const [error, response] = await to(fetch(requestUrl));

        if (error || !response!.ok) {
            console.error(`WFS data failed to load for "${this.config.id}"`, error || response!.statusText);
            return Promise.reject(error || response!.statusText);
        }

        return await response!.json().then(json => {
            let data: WFSResponseJson = json;
...
class CSVSource extends mixins(BlueprintBase, ClientSideData, LatLongOption, FieldsOption) {
    ...
    get layerFactory(): LayerFactory {
        return api.geoapi.layers.makeCsvLayer;
    }
